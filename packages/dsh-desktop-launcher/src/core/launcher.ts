...
function renderPowerShell(spec: LauncherSpec): string {
  return [
    ...
    "$fragmentSeparator = if ($url.Contains('#')) { '&' } else { '#' }",
    "$managedUrl = $url + $fragmentSeparator + 'dsh-launcher=' + [uri]::EscapeDataString($launchToken)",
    '',
    ...
    '}',
    "$previousLaunchToken = [Environment]::GetEnvironmentVariable('DSH_DESKTOP_LAUNCHER_TOKEN', 'Process')",
    "[Environment]::SetEnvironmentVariable('DSH_DESKTOP_LAUNCHER_TOKEN', $launchToken, 'Process')",
    ...
    '}',
    "[Environment]::SetEnvironmentVariable('DSH_DESKTOP_LAUNCHER_TOKEN', $previousLaunchToken, 'Process')",
    '',
...
function renderPosix(platform: 'darwin' | 'linux', spec: LauncherSpec): string {
  ...
  return [
    ...
    "LAUNCH_TOKEN=$(node -e \"process.stdout.write(require('node:crypto').randomBytes(24).toString('hex'))\")",
    "case \"$URL\" in *'#'*) MANAGED_URL=\"$URL&dsh-launcher=$LAUNCH_TOKEN\" ;; *) MANAGED_URL=\"$URL#dsh-launcher=$LAUNCH_TOKEN\" ;; esac",
    '',
    'probe() {',
    ...
    '',
    'if [ -n "$PROFILE" ]; then',
    '  DSH_DESKTOP_LAUNCHER_TOKEN="$LAUNCH_TOKEN" "$DASH" web --profile "$PROFILE" >/dev/null 2>&1 &',
    'else',
    '  DSH_DESKTOP_LAUNCHER_TOKEN="$LAUNCH_TOKEN" "$DASH" web >/dev/null 2>&1 &',
    'fi',
    '',
