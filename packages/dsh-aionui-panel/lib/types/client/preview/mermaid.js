...
export function sanitizeSvg(svg) {
    ...
    for (const el of Array.from(root.querySelectorAll('*'))) {
        for (const attr of Array.from(el.attributes)) {
            if (isEventHandler(attr.name) || isDangerousHref(attr.value))
...
export function findMermaidCodeBlocks(scope) {
    ...
    const shellBlocks = [];
    if (scope instanceof Element && scope.matches('div.md-code-block'))
        shellBlocks.push(scope);
    ...
    for (const block of shellBlocks) {
        ...
        if ((infostring?.textContent ?? '').trim() !== 'mermaid')
            continue;
