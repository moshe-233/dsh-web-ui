...
export function porcelainState(letter) {
    switch (letter) {
...
export function parseStatusView(root, branch, output) {
...
const NO_REPO = { code: 'git-unavailable', message: 'not a git repository' };
/**
...
export class GitService {
    ...
    gitAvailable(signal) {
        if (this.availablePromise === undefined) {
            ...
            cached = this.runner
                ...
                .catch((error) => {
                if (signal?.aborted) {
                    if (this.availablePromise === cached)
    ...
    repoOf(root, signal) {
        ...
        entry.value = this.run(['rev-parse', '--show-toplevel'], root, signal)
            ...
            if (result.exitCode === 127) {
                ...
                if (this.repoCache.get(root) === entry)
                    this.repoCache.delete(root);
                return null;
            }
     ...
     */
    isRepositoryCanonical(canonicalRoot, signal) {
    ...
    status(root, signal) {
        ...
        const clear = () => {
            if (this.statusRequests.get(root) === request)
                this.statusRequests.delete(root);
        };
        void request.then(clear, clear);
        return request;
    }
    async statusFromRoot(root, signal) {
        ...
        if (!(await this.gitAvailable(signal)))
            return null;
        const repo = await this.repo(root, signal);
        if (!repo.ok)
            return repo.error.code === 'git-unavailable' ? null : repo.error;
     ...
     */
    async statusCanonical(canonicalRoot, signal) {
    ...
    statusAt(root, repo, signal) {
        ...
        const clear = () => {
            if (this.statusRuns.get(root) === run)
                this.statusRuns.delete(root);
        };
    ...
    async diff(root, path, staged) {
        ...
        if (!isPathInside(repo.repo, abs))
            return { code: 'path-outside-root', message: 'path outside the repository' };
    ...
    async discard(root, paths) {
        ...
        for (const p of paths) {
            if (inside.includes(join(repo.repo, p)))
                eligible.push(p);
            else
                failed.push(p);
        }
    ...
    async batch(root, paths, op) {
        ...
        if (!ok)
            return { code: 'git-failed', message: 'git operation failed' };
