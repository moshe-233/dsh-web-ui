...
import { isPanelAllowed } from "./access.js";
 ...
 * the host OS.
 */
export function spawnCwd(argv, platform = process.platform) {
    let last = argv[argv.length - 1] ?? process.cwd();
    if (last.startsWith('/select,'))
        last = last.slice('/select,'.length);
...
export async function runGitStatusWithTimeout(run, controller = new AbortController()) {
    let timeout;
...
export function registerPanelRoutes(ctx, fs, git) {
    ...
    const sseWrite = (subscriber, chunk) => {
        ...
        await Promise.all([...subscribers].map(async (subscriber) => {
            ...
            try {
                ...
                const status = await runGitStatusWithTimeout(async (signal) => {
                    if (!(await git.isRepositoryCanonical(subscriber.root, signal)))
                        return null;
    ...
    const sse = async (req, res) => {
        ...
        const subscriber = { root: gated.canonical, lastGit: '', res, closed: false };
